...
public class DynamicParticle : MonoBehaviour
{
    ...
    [SerializeField] public Rigidbody2D rigidBodyDynamic;
    void Awake()
    {
    ...
    public void SetState(STATES newState)
    {
        ...
        { //Only change to a different state
            ...
            {
                case STATES.WATER:
                    rigidBodyDynamic.gravityScale = 1.0f; // To simulate Water density
                    break;
                case STATES.GAS:
                    particleLifeTime = particleLifeTime / 2.0f; // Gas lives the time the other particles
                    rigidBodyDynamic.gravityScale = 0.0f;// To simulate Gas density
                    ...
                    break;
                case STATES.LAVA:
                    rigidBodyDynamic.gravityScale = 0.3f; // To simulate the lava density
                    break;
                case STATES.NONE:
                    Destroy(gameObject);
                    break;
            ...
            if (newState != STATES.NONE)
            {
                ...
                rigidBodyDynamic.velocity = new Vector2();  // Reset the particle velocity	
    ...
    void Update()
    {
        switch (currentState)
        {
            ...
            case STATES.LAVA:
                ...
                break;
            case STATES.GAS:
                if (rigidBodyDynamic.velocity.y < 50)
                { //Limits the speed in Y to avoid reaching mach 7 in speed
                    rigidBodyDynamic.AddForce(new Vector2(0, GAS_FLOATABILITY)); // Gas always goes upwards
                }
                ScaleDown();
                break;
    ...
    void MovementAnimation()
    {
        ...
        movementScale.x += Mathf.Abs(rigidBodyDynamic.velocity.x) / 30.0f;
        movementScale.z += Mathf.Abs(rigidBodyDynamic.velocity.y) / 30.0f;
